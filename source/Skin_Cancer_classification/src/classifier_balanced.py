...
lil_df = dt.balance_data(df)
train_df_bal, test_df_bal, val_df_bal = dt.split_balanced_data(lil_df, args.train_split_bal, args.val_split_bal)
train_df, test_df, val_df = train_df_bal, test_df_bal, val_df_bal
...
def loading_model(pooling, input_shape,nodes_layer_1, nodes_layer_2, activation_hidden_layer, activation_output_layer, initial_learning_rate, decay_steps, decay_rate, loss): 
    # load the pretrained VGG19 model without classifier layers
...
def save_results(test_ds, predictions):
    ...
    outpath_report = os.path.join(os.getcwd(), "out", "classification_report_balanced_vgg19.txt")
